...
<div class="_in-blocks">
	<h4 class="_left">Edit Your Profile</h4>
</div>
{{ Form::model($dev, array('method' => 'PATCH', 'route' => array('devs.update', $dev->id))) }}
	...
	<!-- Email - implement activation of changed emails -->
<!-- 	<div class="control-group{{ $errors->first('email', ' error') }}">
	...
	<div class="control-group{{ $errors->first('pic', ' error') }}">
		...
		<div class="controls">
			<input class="form-control" type="text" name="pic" id="pic" value="{{ Input::old('pic', $dev->pic) }}" />
			{{ $errors->first('pic', '<span class="help-block">:message</span>') }}
		</div>
	</div>
	<!-- video -->
	<div class="control-group{{ $errors->first('video', ' error') }}">
		<label class="control-label" for="video">Youtube Video</label>
		<div class="controls">
	...
	<div class="control-group{{ $errors->first('elevator', ' error') }}">
		...
		<div class="controls">
			<input class="form-control" type="text" name="elevator" id="elevator" value="{{ Input::old('elevator', $dev->elevator) }}" />
	...
	<div class="control-group{{ $errors->first('public', ' error') }} _checkbox">
        {{ Form::checkbox('public', null, Input::old('public', $dev->public), array('class'=> 'form-control')) }}
		{{ Form::label('public', 'Public') }}
        {{ $errors->first('public', '<span class="help-block">:message</span>') }}
	</div>
	<!-- notes -->
	<div class="control-group{{ $errors->first('notes', ' error') }}">
	...
	<div class="control-group">
		<div class="controls _in-blocks">
			{{ link_to_route('devs.show', 'Back', $dev->id, array('class' => 'btn btn-default')) }}
			<button type="submit" class="btn btn-primary _right">Update</button>
		</div>
	</div>
